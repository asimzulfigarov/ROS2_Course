...
public:
    SimpleParameter() : Node("simple_parameter")
    {
        ...
        declare_parameter<std::string>("simple_string_param", "Robotics");

        param_callback_handle = add_on_set_parameters_callback(std::bind(&SimpleParameter::paramChangeCallback, this, _1));
    }
private:
    ...
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(const std::vector<rclcpp::Parameter>& parameters)
    {
        ...
        for (const auto& param: parameters)
        {
            ...
            {
                RCLCPP_INFO_STREAM(get_logger(), "Param simple_int_param changed! New value: " << param.as_string());
                result.successful = true;
